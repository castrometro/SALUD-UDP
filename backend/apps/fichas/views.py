...
class FichaAmbulatoriaViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = FichaAmbulatoria.objects.select_related(
            'paciente', 'creado_por', 'modificado_por', 'estudiante', 'ficha_base'
        )
        
        if user.role not in ['ADMIN', 'DOCENTE']:
            # Estudiante ve: plantillas + sus propias fichas
    ...
    def perform_create(self, serializer):
        ...
        # Si es docente/admin y no especifica estudiante, es plantilla
        if user.role in ['ADMIN', 'DOCENTE']:
            ...
            # Si es estudiante, la ficha es de él
    ...
    def crear_mi_ficha(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(
                FichaAmbulatoriaSerializer(ficha, context={'request': request}).data,
    ...
    def historial(self, request, pk=None):
        ...
        historial = ficha.historial.all()
        serializer = FichaHistorialSerializer(historial, many=True)
    ...
    def mi_ficha(self, request, pk=None):
        ...
        try:
            mi_ficha = FichaAmbulatoria.objects.get(
                ficha_base=ficha_base,
                estudiante=request.user
            )
            serializer = FichaAmbulatoriaSerializer(mi_ficha, context={'request': request})
            return Response(serializer.data)
